...
7. スライサーを追加するには、レポート ページの空の領域をクリックして、画像を選択解除します。
...
15. **Region | Region**フィールドに基づいて、2 番目のスライサーを作成します (階層の**地域** レベルではありません)。  

16. スライサーのサイズを変更し、再配置します。

	![画像 21](Linked_image_Files/PowerBI_Lab08A_image19.png)

17. スライサーを書式設定するため、「**視覚化**」 ウィンドウの下の 「**書式設定**」 ウィンドウを開きます。 
...
25. ビジュアル フィールド ウィンドウで (「**フィールド**」 ウィンドウではなく 「ビジュアル フィールド」 ウィンドウは 「**視覚化**」 ウィンドウの下にあります)、フィールドが 「**共有軸(Shared axis)**」 および 「**列の値（Colmun values）**」 ウェルに割り当てられていることを確認してください。 

	![画像 27](Linked_image_Files/PowerBI_Lab08A_image25.png)

	*ビジュアルをビジュアルにドラッグすると、既定のウェルに追加されます。精度のため、フィールドをウェルに直接ドラッグできます*。

26. 「**フィールド**」 ウィンドウから、「**Sales|Profit Margin**」 フィールドを 「**行の値**」 ウェルにドラッグします。  
