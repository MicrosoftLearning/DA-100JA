...
7. ビジュアルを書式設定するには、「**視覚化（Visualization）**」 ウィンドウの下にある 「**書式設定（Format）**」 ウィンドウを選択します。   

	![画像 48](Linked_image_Files/PowerBI_Lab06B_image4.png)

8. 「**検索**」 ボックスに、「**階段状（Stepped）**」と入力します。

9. 「**階段状レイアウト(Stepped Layout)**」 プロパティを 「**オフ**」 に設定します。   
...
6. **Sales % All Region** メジャーを、小数点以下 2 桁のパーセンテージ表示として書式設定します。 
...
1. レポート ビューの**ページ 2** で、年と月でグループ化されたさまざまなメジャーを表示するマトリックス ビジュアルを確認します。

2. 次の式に基づいて、**Sales** テーブルにメジャーを追加し、小数点以下の桁数が 0 に設定します。
