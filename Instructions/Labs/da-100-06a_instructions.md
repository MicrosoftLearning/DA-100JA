...
6. **Salesperson | EmployeeKey**から**Sales | EmployeeKey**へのリレーションシップを作成します。
...
9. 「**Salesperson**」 テーブルで、次の列を非表示にします。
...
11. 「**プロパティ**」 ペインの 「**説明 (Description)**」 ボックスに、次のように入力します。**Salesperson related to s sale**

	*ユーザーがテーブルまたはフィールドにカーソルを合わせると、その説明が 「**フィールド**」 ウィンドウにツール チップとして表示されることを思い出してください*。

12. 「**Salesperson (Perfomance)**」 テーブルの説明(Description）を次の値に設定します。**Salesperson related to region(s)**
...
3. 「**テーブル ツール**」 コンテキスト リボンの 「**カレンダー**」  グループ内から、「**日付テーブルとしてマーク(Mark as date table)**」 をクリックし、「**日付テーブルとしてマーク**」 を選択します。       
...
2. ビジュアル フィールド ウィンドウ (**視覚化(Visualizations)** ウィンドウの下) の**値**ウェルに、**UnitPrice**が一覧表示されていることに注目してください。
...
13. 「**Order**」 メジャーと 「**Order Lines**」 メジャーを複数選択し、次の要件を設定します。   
	...
	* 桁区切り記号を使用する書式を設定する(Thousands sepatator)

	* 「**Counts**」 という名前の表示フォルダーに割り当てる

	![画像 36](Linked_image_Files/PowerBI_Lab06A_image33.png)

14. レポート ビューにある、マトリックス ビジュアルの 「**Value**」 ウェルで、「**Unit Price**」 フィールドの 「X」 をクリックして削除します。   
...
6. **Target**メジャーを小数点以下の桁数がゼロの書式設定します。 
...
10. スニペット ファイル定義を使用して、「**Target**」 テーブルに次の 2 つのメジャーを作成します。

	* Variance

	* Variance Margin

11. 「**Variance**」 メジャーを小数点以下の桁数がゼロに書式設定します。 

12. 「**Variance Margin**」 メジャーを小数点以下 2 桁に書式設定します。 
