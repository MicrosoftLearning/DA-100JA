...
2. 7 つのテーブルがすべて表示されない場合は、右にスクロールし、テーブルをまとめてドラッグして整列して、すべてのテーブルを同時に表示できるようにします。
...
16. 「**このリレーションシップをアクティブにする(Make this relationship active)**」 がオンになっていることを確認します。 
...
2. 階層を作成するには、「**フィールド**」 ウィンドウで 「**Category**」 列を右クリックし、「**階層の作成(Create hierarchy)**」 を選択 します。
...
3. 「**プロパティ**」 ペインで 「**詳細(Advanced)**」 セクションを展開し、「**データ カテゴリ**」 ドロップダウン リストから 「**国/地域(Country/Region)**」 を選択します。
...
4. **プロパティ** ペインの **書式設定(formatting)** セクション内から、**桁区切り記号(Thousands separator)** プロパティを **はい** にスライドします。

	![画像 357](Linked_image_Files/PowerBI_Lab04A_image28.png)

5. 「**UnitPrice**」 列を選択します。 

6. 「**プロパティ**」 ペインの 「**書式設定**」 セクション内で、「**小数点以下表示桁数(Decimal places)**」 プロパティを **2** にスライドします。

7. 「**詳細**」 グループ (下にスクロールして配置する必要がある場合があります) の 「**集計の方法(Summarize by)**」 ドロップダウン リストで、「**平均(Average)**」 を選択します。
...
2. 「**プロパティ**」 ペインで、「**非表示(Is hidden)**」 プロパティを 「**はい**」 にスライドします。
...
2. 「**クイック メジャー**」 ウィンドウの 「**計算**」 ドロップダウン リストで、「**数値演算(Matgenatucak operations)**」 グループ内から 「**減算(Subtraction)**」 を選択します。

	![画像 367](Linked_image_Files/PowerBI_Lab04A_image39.png)

3. 「**フィールド**」 ペインで、「**Sales**」 テーブルを展開します。

4. 「**Sales**」 フィールドを 「**基準値(Base value)**」 ボックスにドラッグします。   
